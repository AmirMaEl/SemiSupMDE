...
def get_scheduler(optimizer, lr_policy='exponent'):
    if lr_policy == 'lambda':
...
def init_net(net, init_type='normal', gpu_ids=[],accelerator=None):
...
def define_G(input_nc, output_nc, ngf=64, layers=4, norm='batch', activation='PReLU', model_type='UNet',
                    init_type='xavier', drop_rate=0, add_noise=False, gpu_ids=[], weight=0.1,accelerator=None):

    if model_type == 'ResNet':
        net = _ResGenerator(input_nc, output_nc, ngf, layers, norm, activation, drop_rate, add_noise, gpu_ids)
    elif model_type == 'UNet':
        net = _UNetGenerator(input_nc, output_nc, ngf, layers, norm, activation, drop_rate, add_noise, weight)
    ...
    return init_net(net, init_type, gpu_ids, accelerator)

# define the discriminator network
def define_D(input_nc, ndf = 64, n_layers = 3, num_D = 1, norm = 'batch', activation = 'PReLU', init_type='xavier', gpu_ids = [], accelerator=None):
...
def define_featureD(input_nc, n_layers=2, norm='batch', activation='PReLU', init_type='xavier', gpu_ids=[], accelerator=None):
